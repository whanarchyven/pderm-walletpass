...
export interface GoogleWalletPassEventServiceOptions {
  credentials: any;
  ticket: {
    issuerId: string;
    classSuffix: string;
...
export class GoogleWalletPassEventService extends PreConfiguredService<GoogleWalletPassEventServiceOptions> {
  httpClient: any;
  baseUrl = "https://walletobjects.googleapis.com/walletobjects/v1";
  batchUrl = "https://walletobjects.googleapis.com/batch";
  classUrl!: string;
  objectUrl!: string;
  constructor(config: GoogleWalletPassEventServiceOptions) {
    /**
  ...
  createJwtNewObjects(
    ...
    objectSuffix,
    {
      ...
      qrCodeData,
      personStatus,
    }: { ticketHolderName: string; qrCodeData: string; personStatus: string }
  ) {
    // See link below for more information on required properties
    // https://developers.google.com/wallet/tickets/events/rest/v1/eventticketclass
    // let newClass = {
    ...
    let newObject = {
      ...
      // heroImage: {
      //   sourceUri: {
      //     uri: "https://pediatric2.d.sinc.ru/upload/iblock/254/rs18zur047o5yowl60jwpoecx277k352.JPG",
      //   },
      //   contentDescription: {
      //     defaultValue: {
      //       language: "en-US",
    ...
    let token = jwt.sign(claims, this.config.credentials.private_key, {
      algorithm: "RS256",
    });
